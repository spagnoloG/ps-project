...
void print_boddies(Body **bodies, int n, int iteration, FILE *logfile) {
  for (int i = 0; i < n; i++) {
    fprintf(logfile, "%d,%f,%f,%f,%f,%f,%f,%f\n", iteration, bodies[i]->mass,
            bodies[i]->x, bodies[i]->y, bodies[i]->z, bodies[i]->vx,
            bodies[i]->vy, bodies[i]->vz);
  }
}
...
void progress_bar(int current, int max) {
  ...
  int width = w.ws_col;
  float progress =
      (float)current / (float)max; // Calculate progress as a fraction
  int chars_printed =
      (int)(progress * width); // Calculate number of characters to print
                               //
 ...
 * Algorithm
 */
Body **generate_initial_population(int n, float max_mass, float max_pos,
                                   float max_vel) {
...
#pragma omp parallel for schedule(guided, 20)
  for (int i = 0; i < num_bodies; i++) {
    ...
    for (int j = 0; j < num_bodies; j++) {
      ...
      forceX += bodies[j]->mass * rx /
                (pow(distance, 3) + EPSILON); // G je epsilon xd
      forceY += bodies[j]->mass * ry /
                (pow(distance, 3) + EPSILON); // G je epsilon xd
      forceZ += bodies[j]->mass * rz /
                (pow(distance, 3) + EPSILON); // G je epsilon xd
    }
    ...
    new_bodies[i]->x =
        bodies[i]->x + bodies[i]->vx * DT + 0.5 * ax * pow(DT, 2);
    new_bodies[i]->y =
        bodies[i]->y + bodies[i]->vy * DT + 0.5 * ay * pow(DT, 2);
    new_bodies[i]->z =
        bodies[i]->z + bodies[i]->vz * DT + 0.5 * az * pow(DT, 2);
  }
